...
from engagements.models import Tenant, Ticket, Comment, Attachment
...
def create_work_item(tenant, user, **kwargs):
    """
    Create a WorkItem of the correct subclass (Ticket, Case, or Job) based on the tenant's work_item_type.
    """
    work_item_type = getattr(tenant, 'work_item_type', 'ticket')
    base_kwargs = {
        'tenant': tenant,
        'created_by': user,
    ...
    if work_item_type == 'ticket':
        ...
        raise ValueError(f"Unknown work_item_type: {work_item_type}")
