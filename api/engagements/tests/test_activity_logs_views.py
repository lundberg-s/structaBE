...
from engagements.models import Ticket, ActivityLog
...
class ActivityLogViewTests(APITestCase):
    ...
    def test_activity_log_list_unauthenticated(self):
        url = reverse('engagements:activity-log-list-create')
        response = self.client.get(url)
        assert response.status_code == status.HTTP_401_UNAUTHORIZED

    def test_activity_log_retrieve_not_found(self):
        url = reverse('engagements:activity-log-detail', kwargs={'id': '00000000-0000-0000-0000-000000000000'})
        response = self.client.get(url)
