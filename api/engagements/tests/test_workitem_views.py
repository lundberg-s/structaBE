...
from engagements.tests.factory import create_tenant, create_user, create_work_item, create_comment, create_attachment
from engagements.models import Ticket, Comment, Attachment
import uuid
...
class WorkItemViewTests(APITestCase):
    ...
    def test_create_and_fetch_ticket_with_comment_and_attachment(self):
        ...
        # Fetch ticket detail
        response = self.client.get(reverse('engagements:current-work_item-detail', kwargs={'pk': work_item.id}))
    ...
    def test_filtering_and_search(self):
        ...
        # Filter by status
        response = self.client.get(reverse('engagements:current-work_item-list'), {'status': 'open'})
        self.assertEqual(response.status_code, 200)  # type: ignore
        ...
        # Search by title
        response = self.client.get(reverse('engagements:current-work_item-list'), {'search': 'Beta'})
        self.assertEqual(response.status_code, 200)  # type: ignore
