...
class TestTicketFlow(EngagementsTestHelper):
    ...
    def test_list_tickets_returns_correct_count(self):
        ...
        url = reverse(TestURLs.TICKET_LIST)
        response = self.client.get(url)
        self.assertEqual(len(response.data), ExpectedResults.RESULT_COUNT)  # 3 tickets from setUp
    ...
    def test_list_tickets_scoped_to_tenant(self):
        ...
        url = reverse(TestURLs.TICKET_LIST)
        response = self.client.get(url)
        self.assertEqual(len(response.data), ExpectedResults.RESULT_COUNT)  # Only current tenant tickets
    ...
    def test_filter_by_priority_returns_200(self):
        ...
        url = reverse(TestURLs.TICKET_LIST)
        response = self.client.get(url, {QueryParams.PRIORITY_LABEL: other_priority.label})
        self.assertEqual(response.status_code, 200)
    ...
    def test_filter_by_priority_returns_correct_count(self):
        ...
        url = reverse(TestURLs.TICKET_LIST)
        response = self.client.get(url, {QueryParams.PRIORITY_LABEL: unique_priority.label})
    ...
    def test_filter_by_category_returns_200(self):
        ...
        url = reverse(TestURLs.TICKET_LIST)
        response = self.client.get(url, {QueryParams.CATEGORY_LABEL: other_category.label})
        self.assertEqual(response.status_code, 200)
    ...
    def test_filter_by_category_returns_correct_count(self):
        ...
        url = reverse(TestURLs.TICKET_LIST)
        response = self.client.get(url, {QueryParams.CATEGORY_LABEL: unique_category.label})
    ...
    def test_search_returns_200(self):
        ...
        url = reverse(TestURLs.TICKET_LIST) + f"?{QueryParams.SEARCH}={TestData.UNIQUE_TITLE}"
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
    ...
    def test_filter_and_search_returns_200(self):
        ...
        url = (
            ...
            + f"?status={QueryParams.STATUS_OPEN}&priority={QueryParams.PRIORITY_HIGH}&{QueryParams.SEARCH}={TestData.MY_TENANT_TITLE}"
        )
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
