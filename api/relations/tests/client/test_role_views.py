...
class TestRoleViews(FullySetupTest):
    def setUp(self):
        ...
        self.admin_role = create_role(self.tenant, label="Admin", is_system=False)
        self.user_role = create_role(self.tenant, label="User", is_system=False)
    ...
    def test_create_system_role(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        role = Role.objects.get(id=response.data['id'])
        self.assertEqual(role.label, 'System Role')
    ...
    def test_role_unique_constraint(self):
        self.authenticate_client()
        # Create a role with the same label in the same tenant
        create_role(self.tenant, label="Duplicate Role", is_system=False)
        data = {
    ...
    def test_system_role_unique_constraint(self):
        ...
        create_role(self.tenant, label="System Role", is_system=True)
        data = {
            'label': 'System Role',
            'is_system': True,
        }
    ...
    def test_retrieve_role(self):
        self.authenticate_client()
        url = reverse('relations:role-detail', args=[self.admin_role.id])
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
